...
-- Ultra Blatant V2 Category (OPTIMIZED)
...
local savedUltraBlatantCancelDelay = GetConfigValue("UltraBlatant.CancelDelay", 0.05)

-- ⭐ Cache module reference for better performance
local UltraBlatantModule = nil
local function getUltraBlatant()
    if not UltraBlatantModule then
        UltraBlatantModule = GetModule("UltraBlatant")
    end
    return UltraBlatantModule
end

-- ⭐ Apply settings to module immediately on load (optimized)
task.spawn(function()
    task.wait(0.3)  -- Reduced from 0.5
    local UltraBlatant = getUltraBlatant()
    if UltraBlatant then
        -- Use UpdateSettings for consistency
        if UltraBlatant.UpdateSettings then
        ...
        elseif UltraBlatant.Settings then
...
ToggleReferences.UltraBlatant = makeToggle(catUltraBlatant, "Blatant Mode", function(on)
    ...
    -- Removed SaveCurrentConfig() for better performance - will save on close
...
-- ⭐ OPTIMIZED: Cached module reference
makeInput(catUltraBlatant, "Complete Delay", savedUltraBlatantCompleteDelay, function(v)
    ...
    -- Removed SaveCurrentConfig() for better performance
...
-- Fast Auto Fishing Perfect Category (OPTIMIZED)
local catBlatantV2Fast = makeCategory(mainPage, "Fast Auto Fishing Perfect", "🔥")

-- ⭐ Cache module reference for better performance
local BlatantV2Module = nil
local function getBlatantV2()
    if not BlatantV2Module then
        BlatantV2Module = GetModule("blatantv2")
    end
    return BlatantV2Module
end
